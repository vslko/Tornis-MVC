...
class XLSXWriterLight
{
	...
	protected function initializeSheet($sheet_name)
	{
		...
		$this->sheets[$sheet_name] = (object)array(
			...
			'row_count' => 0,
			'file_writer' => new XLSXWriterLight_BuffererWriter($sheet_filename),
			'columns' => array(),
			'merge_cells' => array(),
		...
		$max_cell=XLSXWriterLight::xlsCell(self::EXCEL_2007_MAX_ROW, self::EXCEL_2007_MAX_COL);//XFE1048577
	...
	protected function writeCell(XLSXWriterLight_BuffererWriter &$file, $row_number, $column_number, $value, $cell_format_index)
	{
		...
		else if ($value{0}!='0' && $value{0}!='+' && is_int($value)){
			$file->write('<c r="'.$cell_name.'" s="'.$cell_format_index.'" t="n"><v>'.($value*1).'</v></c>');
		}
	...
	protected function writeStylesXML()
	{
		...
		$file = new XLSXWriterLight_BuffererWriter($temporary_filename);
	...
	{
		$temporary_filename = $this->tempFilename();
		$file = new XLSXWriterLight_BuffererWriter($temporary_filename, $fd_flags='w', $check_utf8=true);
		$file->write('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'."\n");
...
class XLSXWriterLight_BuffererWriter
{
	protected $fd=null;
	protected $buffer='';
	...
	public function __construct($filename, $fd_fopen_flags='w', $check_utf8=false)
	{
		...
		if ($this->fd===false) {
			XLSXWriterLight::log("Unable to open $filename for writing.");
		}
	}
	...
	protected function purge()
	{
		if ($this->fd) {
			if ($this->check_utf8 && !self::isValidUTF8($this->buffer)) {
				XLSXWriterLight::log("Error, invalid UTF8 encoding detected.");
